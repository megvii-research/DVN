...
class Encoder(nn.Module):
    def __init__(self, n_feat, kernel_size, reduction, act, bias, scale_unetfeats, atten):
        ...
        if self.atten:  # feature attention
            self.atten_enc1 = nn.Sequential(Conv(n_feat, n_feat, 1, bias=bias), act)
            self.atten_enc2 = nn.Sequential(Conv(n_feat+scale_unetfeats, n_feat+scale_unetfeats, 1, bias=bias), act)
            self.atten_enc3 = nn.Sequential(Conv(n_feat+(scale_unetfeats*2), n_feat+(scale_unetfeats*2), 1, bias=bias), act)

            self.atten_dec1 = nn.Sequential(Conv(n_feat, n_feat, 1, bias=bias), act)
            self.atten_dec2 = nn.Sequential(Conv(n_feat+scale_unetfeats, n_feat+scale_unetfeats, 1, bias=bias), act)
            self.atten_dec3 = nn.Sequential(Conv(n_feat+(scale_unetfeats*2), n_feat+(scale_unetfeats*2), 1, bias=bias), act)

            self.nir_edge_1 = Edge(n_feat)
            self.nir_edge_2 = Edge(n_feat+scale_unetfeats)
            self.nir_edge_3 = Edge(n_feat+scale_unetfeats*2)

    def forward(self, x, encoder_outs=None, decoder_outs=None, mask=None):
        if not self.atten or mask is None:
            ...
            assert decoder_outs is not None
            assert mask is not None

            nir_feat_1 = self.atten_enc1(encoder_outs[0]) + self.atten_dec1(decoder_outs[0])
            nir_feat_2 = self.atten_enc2(encoder_outs[1]) + self.atten_dec2(decoder_outs[1])
            nir_feat_3 = self.atten_enc3(encoder_outs[2]) + self.atten_dec3(decoder_outs[2])

            nir_feat_1 = self.nir_edge_1(nir_feat_1)
            nir_feat_2 = self.nir_edge_2(nir_feat_2)
            nir_feat_3 = self.nir_edge_3(nir_feat_3)

            enc1 = self.encoder_level1(x)
            enc1_fuse_nir = enc1 + mask[0] * nir_feat_1
            x = self.down12(enc1_fuse_nir)
            enc2 = self.encoder_level2(x)
            enc2_fuse_nir = enc2 + mask[1] * nir_feat_2
            x = self.down23(enc2_fuse_nir)
            enc3 = self.encoder_level3(x)
            enc3_fuse_nir = enc3 + mask[2] * nir_feat_3

            return [enc1_fuse_nir, enc2_fuse_nir, enc3_fuse_nir], [nir_feat_1, nir_feat_2, nir_feat_3]
...
class Structure(nn.Module):
    def __init__(self, n_feat, kernel_size, reduction, act, bias, scale_unetfeats):
        ...
        self.csff_2 = CrossStageFF(n_feat + scale_unetfeats, scale_unetfeats, kernel_size, reduction, act, bias)
        self.csff_3 = CrossStageFF(n_feat + scale_unetfeats * 2, scale_unetfeats, kernel_size, reduction, act, bias)
